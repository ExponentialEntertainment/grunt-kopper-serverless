...
module.exports = {
	...
	buildEndpoint: function (region, account, title, endpoints) {
		var self = this;

		var methods = _.mapObject(endpoints, function (endpoint) {
			return self.buildMethod(region, account, endpoint.consumes, title + '-' + endpoint.lambda, endpoint.parameters);
		});
	...
	buildApi: function (region, account, host, title, paths) {
		...
		return {
			...
			paths: _.mapObject(paths, function (endpoints) {
				return self.buildEndpoint(region, account, title, endpoints);
			}),
			definitions: {
				Empty: {
					type: 'object'
